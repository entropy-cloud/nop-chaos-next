...
function applyThemeToDocument(config: ThemeConfig) {
  ...
  const resolvedMode = resolveMode(resolved.displayMode);
...
### 2.2.1 Canonical Theme Fallback

当前 live baseline 只有一套 canonical 主题解析语义：`apps/main/src/config/themeResolution.ts` 中的 `resolveThemeConfig()`。

- 若持久化或外部输入的 `themeId` 未注册，解析结果会回退到默认主题，而不是保留失效 id
- `themeStore` hydrate merge、`applyThemeToDocument()`、以及 `App.tsx` 注入给 plugin bridge 的 `getThemeConfig()` / bridge snapshot 都使用同一套解析结果
- 因此失效主题场景下，DOM `data-theme`、store 可见主题、设置页高亮和 plugin bridge 观察值保持一致，不再出现“DOM 已回退但状态仍保留旧 id”的分叉
...
- canonical owner 是 `@nop-chaos/ui` 的 `flux-lib/ui/src/components/ui/table-row-class-name.ts`
- `apps/main` 业务页面直接从 `@nop-chaos/ui` 导入该 helper，不再维护本地副本
- `subtle` 变体当前基线固定为 `hover:bg-surface-hover`，避免 app 与 shared UI 分叉
...
- `amis-theme-bridge.css` 只映射 AMIS 自己消费的变量，不允许在局部作用域重写宿主语义 token（如 `--background`）
- 不全局引入 `amis/lib/helper.css`（会导致类名冲突，详见 `docs/12-amis-helper-css-conflict.md`）
- 新增 Amis 组件适配写在 `amis-fix.css` 中
- dialog / modal overlay 与 shared UI overlay 统一使用 `--surface-overlay` 对应的语义色，而不是 `black/xx` 裸色
...
当前 live baseline 中，`subtle` 统一使用 `hover:bg-surface-hover`，由主题 token 决定浅色/暗色表现，不再在组件内硬编码白色或 slate 色值。
