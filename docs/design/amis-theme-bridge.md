...
3. **宿主语义 token 不可在 bridge 中重定义**：`amis-theme-bridge.css` 只能写 AMIS 自己消费的变量，例如 `--colors-*`、`--Tooltip-*`、`--common-popover-border`。不要在 `.amis` 作用域内把宿主 `--background` 之类的语义 token 改写成 `card` 含义。

4. **新增组件适配**：遇到样式不一致的组件可在 `amis-fix.css` 中添加覆盖；若需要 card/popover 语义，直接在消费处写 `hsl(var(--card))` 或 `var(--surface-overlay)`，不要借道覆盖宿主 token。
