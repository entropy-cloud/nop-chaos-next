...
- [ ] Modify `playwright.config.ts` to read `BASE_URL` env var with fallback to `PLAYWRIGHT_BASE_URL`:
  ```typescript
  ...
  ```
- [ ] Update the `useExternalServer` logic accordingly
- [ ] Verify: `BASE_URL=http://external:4175 pnpm test:e2e -- --grep "can start from login"` uses external server
- [ ] Verify: `PLAYWRIGHT_BASE_URL=http://external:4175 pnpm test:e2e -- --grep "can start from login"` still works (backward compat)
- [ ] Verify: no `BASE_URL` or `PLAYWRIGHT_BASE_URL` set uses default `http://127.0.0.1:4175`

Exit Criteria:

> All `[x]` before Phase 1 Status can be set to `completed`.

- [ ] `BASE_URL` env var is recognized and overrides the default base URL
- [ ] `PLAYWRIGHT_BASE_URL` continues to work as before (backward compatible)
- [ ] When `BASE_URL` and `PLAYWRIGHT_BASE_URL` are both set, `BASE_URL` takes precedence
- [ ] When neither is set, default `http://127.0.0.1:4175` is used
- [ ] `useExternalServer` correctly skips the built-in webServer when either variable is set
- [ ] `pnpm typecheck` passes
- [ ] No owner-doc update required (env var is self-documenting in config)
- [ ] `docs/logs/` 对应日期条目已更新
...
- [ ] Add `@nop-chaos/e2e-shared` as a devDependency in root `package.json` (or create minimal `tests/e2e/package.json` if needed)
- [ ] Verify `packages/e2e-shared/fixtures.ts` exports a `test` fixture with engine injection
- [ ] Update `playwright.config.ts` to read `E2E_ENGINE` env var and pass it to the environment (for engine factory to pick up)
- [ ] Verify: `pnpm typecheck` passes
- [ ] Verify: `pnpm build` passes
- [ ] Verify smoke test: `E2E_ENGINE=flux pnpm test:e2e` at least does not crash (even if no tests use the fixture yet)

Exit Criteria:

> All `[x]` before Phase 2 Status can be set to `completed`.

- [ ] `@nop-chaos/e2e-shared` is a resolvable dependency from the e2e test context
- [ ] `E2E_ENGINE` env var is recognized at config level
- [ ] `pnpm typecheck` passes
- [ ] `pnpm build` passes
- [ ] `pnpm test:e2e` still works for existing specs
- [ ] `E2E_ENGINE=flux pnpm test:e2e` does not crash (gracefully handles no-op when no specs use engine fixture)
- [ ] No owner-doc update required (design doc already documents the env var schema)
- [ ] `docs/logs/` 对应日期条目已更新
...
- [ ] Both phases completed with Exit Criteria checked
- [ ] `BASE_URL` env var works correctly
- [ ] `PLAYWRIGHT_BASE_URL` backward compatibility preserved
- [ ] `E2E_ENGINE` env var recognized
- [ ] `@nop-chaos/e2e-shared` resolvable from e2e test context
- [ ] `pnpm typecheck` passes
- [ ] `pnpm build` passes
- [ ] `pnpm lint` passes
- [ ] `pnpm test:e2e` passes (all existing specs)
- [ ] No owner-doc update required (env var schema documented in design docs)
