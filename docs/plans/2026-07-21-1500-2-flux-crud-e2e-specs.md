...
- [ ] 设计模拟 CRUD 实体的数据结构（字段、类型、验证规则），使其同时适用于 AMIS 和 Flux 渲染
- [ ] 创建共享的 mock 数据工厂函数（或在各 spec 中分别定义）
- [ ] 定义 route interception 模式（列表查询 / 新增 / 编辑 / 删除对应的 GraphQL 或 REST mock 端点）
- [ ] 确认 mock 响应格式与 `CrudListPage` 和 `FormDialog` 的行为期望一致

Exit Criteria:

> 每个 Phase 完成后，必须逐条勾选本节。所有 `[x]` 后才能将 Phase Status 改为 `completed`。

- [ ] Mock 数据结构和 interception 模式已定义
- [ ] Flux 渲染下 mock 数据格式已验证（可以参考 `flux-prototype.spec.ts` 的 mock 模式）
- [ ] `No owner-doc update required` (new test specs, no design change)
- [ ] `docs/logs/` 对应日期条目已更新
...
- [ ] 实现文本输入字段测试：`input[type="text"]` / `textarea` / `input[type="number"]` 在 Flux 下的交互
- [ ] 实现下拉选择测试：单选 `selectOption`、多字段标签 `selectOption(fieldLabels, optionTexts)`、级联选择（如适用）
- [ ] 实现日期选择测试：`datePickerSelect` 日历选择、回退到 `input[type="date"]` 填充
- [ ] 实现表单提交/重置测试：submit button、form field 验证
- [ ] 验证 `E2E_ENGINE=flux` 下全部通过
- [ ] 验证 `E2E_ENGINE=amis` 下被跳过

Exit Criteria:

> 每个 Phase 完成后，必须逐条勾选本节。所有 `[x]` 后才能将 Phase Status 改为 `completed`。

- [ ] `tests/e2e/flux-crud-form.spec.ts` 创建完成
- [ ] `E2E_ENGINE=flux pnpm test:e2e -- tests/e2e/flux-crud-form.spec.ts` 通过
- [ ] `pnpm typecheck` 通过
...
- [ ] 实现列表渲染测试：验证表格可见、行数正确、数据加载
- [ ] 实现行操作测试：验证编辑按钮和删除按钮可见并可点击
- [ ] 实现新增按钮测试：验证新增按钮可见，点击后可打开表单
- [ ] 使用 `CrudListPage` 的 `navigate()`、`waitForList()`、`findRowByText()`、`getAddButton()`、`editRow()`、`deleteRow()`
- [ ] 验证 `E2E_ENGINE=flux` 下全部通过
...
- [ ] `tests/e2e/flux-crud-list.spec.ts` 创建完成
- [ ] `E2E_ENGINE=flux pnpm test:e2e -- tests/e2e/flux-crud-list.spec.ts` 通过
- [ ] `pnpm typecheck` 通过
- [ ] `No owner-doc update required`
- [ ] `docs/logs/` 对应日期条目已更新
...
- [ ] 实现完整 CRUD 流程测试：打开列表 → 点击新增 → 填写表单 → 提交 → 验证列表更新
- [ ] 实现编辑流程测试：选择行 → 编辑 → 修改字段 → 提交 → 验证列表更新
- [ ] 实现删除流程测试：选择行 → 删除 → 确认 → 验证行从列表中消失
- [ ] 使用 `CrudListPage` + `FormDialog` 的组合 API，模拟真实用户操作序列
- [ ] 验证 `E2E_ENGINE=flux` 下全部通过
...
- [ ] `tests/e2e/flux-crud-flow.spec.ts` 创建完成
- [ ] `E2E_ENGINE=flux pnpm test:e2e -- tests/e2e/flux-crud-flow.spec.ts` 通过
- [ ] `pnpm typecheck` 通过
- [ ] `No owner-doc update required`
- [ ] `docs/logs/` 对应日期条目已更新
...
- [ ] 确认 Phase 1-4 全部 `completed`
- [ ] 运行 `E2E_ENGINE=flux pnpm test:e2e -- tests/e2e/flux-crud*.spec.ts` 全部通过
- [ ] 运行 `E2E_ENGINE=amis pnpm test:e2e -- tests/e2e/flux-crud*.spec.ts` 确认跳过
- [ ] 运行 `pnpm test:e2e`（AMIS 默认模式）确认现有 63/74 结果无回归
- [ ] 运行 `pnpm typecheck && pnpm build && pnpm lint && pnpm test` 全绿
- [ ] 捕获 deferred items 和 non-blocking follow-ups
- [ ] 安排独立子 agent closure audit
...
- [ ] 3 个 flux-crud spec 文件已创建并通过 `E2E_ENGINE=flux` 测试
- [ ] `E2E_ENGINE=amis` 下新 spec 被跳过
- [ ] 现有 AMIS CRUD spec（master-detail-buttons + master-detail-dialogs）无回归
- [ ] `pnpm test:e2e` 全量 74 个测试用例（23+ spec 文件）中至少 65 通过（当前 baseline 水平，无新增 regression）
