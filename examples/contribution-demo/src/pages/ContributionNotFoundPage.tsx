...
const quickLinks = [
  {
    ...
    description: 'Return to the contributed dashboard page and continue browsing from the default Harbor workspace.',
    path: '/'
  },
  {
    title: 'Open Harbor page',
    description: 'Jump directly to the contribution builtin page registered by the Harbor demo package.',
...
export function ContributionNotFoundPage() {
  return (
            ...
            <div className='flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between'>
              <div className='space-y-4'>
                <div className='inline-flex items-center gap-3 rounded-full border border-teal-200/70 bg-white/80 px-4 py-2 text-sm font-medium text-teal-700'>
                  ...
                  Harbor contribution 404
                </div>
                <div className='space-y-3'>
                  <div className='eyebrow-text tracking-[0.24em] text-teal-700'>System Page Override</div>
                  <CardTitle className='max-w-3xl text-4xl leading-tight text-slate-950'>This route is not mapped inside the Harbor workspace.</CardTitle>
                  <p className='max-w-3xl text-base leading-7 text-slate-600'>
                    The host shell routed you to `/404`, but the rendered screen comes from the contribution package. This
                    demonstrates a second system page override beyond login.
                  </p>
                </div>
              </div>
          ...
          <CardContent className='mt-8 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]'>
            ...
            <div className='rounded-[1.5rem] border border-slate-200/80 bg-slate-950 p-6 text-slate-50 shadow-sm'>
              ...
              <div className='mt-5 space-y-3 text-sm leading-7 text-slate-300'>
                <p>Login override proves that a contribution can replace entry flows.</p>
                <p>404 override proves that the host can keep route ownership while delegating system views.</p>
                <p>The same shell still handles auth state, menu loading, permissions, and plugin runtime coordination.</p>
              </div>
              <Button className='mt-6 w-full justify-center bg-emerald-400 text-slate-950 hover:bg-emerald-300' onClick={() => navigateTo('/')}>
