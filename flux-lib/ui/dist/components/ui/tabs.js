...
function Tabs({ className, orientation = 'horizontal', ...props }) {
    return (_jsx(TabsPrimitive.Root, { "data-slot": "tabs", "data-orientation": orientation, className: cn(orientation === 'horizontal' ? 'group/tabs flex flex-col gap-2' : 'group/tabs flex gap-2', className), ...props }));
}
...
function TabsContent({ className, keepMounted = true, ...props }) {
    return (_jsx(TabsPrimitive.Panel, { "data-slot": "tabs-content", className: cn('flex-1 text-sm outline-none', className), keepMounted: keepMounted, ...props }));
}
