  ...
  return page.evaluate(
    (propNames) => {
      const sidebar = document.querySelector(
        '[class*="h-screen"][class*="overflow-hidden"][class*="border-r"]',
      );
      if (!sidebar) return {};
...
test.describe('AMIS CSS isolation', () => {
  test('sidebar styles should be stable after AMIS CSS loads', async ({ page }) => {
    ...
    await expect(page.getByText('Amis Preview', { exact: false }).first()).toBeVisible({
      timeout: 15000,
    });
    ...
    const ROUTE_ACTIVE_PROPS = new Set(['color', 'backgroundColor', 'borderColor', 'borderRadius']);
