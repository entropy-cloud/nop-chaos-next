...
function CollapsedSidebarFlyout({ item, activePath, onNavigate }: CollapsedSidebarFlyoutProps) {
  ...
  return (
    <div className="w-64 space-y-2">
      <div className="border-b border-[hsl(var(--border))] pb-2 text-sm font-semibold text-foreground">{title}</div>
      {children.length > 0 ? (
...
function SidebarItem({ item, activePath, expandedIds, collapsed, depth, onNavigate, onToggleGroup }: SidebarItemProps) {
  ...
  const itemButton = (
    ...
    >
      ...
      {collapsed ? null : <span className="min-w-0 flex-1 truncate whitespace-nowrap">{getMenuLabel(item)}</span>}
    </button>
  )
