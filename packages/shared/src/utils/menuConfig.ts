import { isAppIconName } from '../types/icon'
...
function validateMenuItem(value: unknown, fieldPath: string): MenuItem {
  ...
  if (value.icon !== undefined && (typeof value.icon !== 'string' || !isAppIconName(value.icon))) {
  ...
  if (pageType === 'builtin' && !componentId) {
    throw new Error(`Invalid menu config: '${fieldPath}.componentId' is required for builtin pages`)
  }
  ...
  return {
    ...
    titleKey,
    path,
    icon: value.icon as MenuItem['icon'] | undefined,
    children,
    badge,
    pageType,
    componentId,
