...
export function normalizeRequestUrl(url: string, query?: Record<string, unknown>) {
  const nextUrl = appendQueryParams(new URL(url, isAbsoluteUrl(url) ? undefined : getBaseOrigin()), query)
...
export function resolveRequestUrl(url: string, query: Record<string, unknown> | undefined, baseUrl: string) {
