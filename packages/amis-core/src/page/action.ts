...
function resolveModuleUrl(modulePath: string, schemaPath?: string) {
  ...
  if (schemaPath && (isAbsoluteHttpUrl(schemaPath) || !splitPrefixUrl(schemaPath))) {
    const baseUrl = isAbsoluteHttpUrl(schemaPath) ? schemaPath : new URL(schemaPath, getBaseOrigin()).href
...
function normalizeImportMap(modulePaths: unknown) {
  ...
  if (isRecord(modulePaths)) {
    return Object.entries(modulePaths).reduce<Record<string, string>>((accumulator, [key, value]) => {
...
async function createImportedScope(modulePaths: unknown, standalone: boolean, page: AmisPageObject): Promise<ImportedScope> {
...
function processActionString(value: string, page: AmisPageObject, scopes: ImportedScope[]) {
  ...
  if (prefixedUrl) {
    ...
    if (type === 'query' || type === 'mutation' || type === 'subscription' || type === 'graphql' || type === 'dict' || type === 'page') {
      return `${type}://${path}`
    }
  }
  ...
  if (value.startsWith('@action:')) {
    ...
    const action = resolveImportedAction(lookupName, scopes, page) ?? adapter.resolveAction?.(lookupName, page)
...
async function bindValue(value: unknown, page: AmisPageObject, scopes: ImportedScope[]): Promise<unknown> {
  ...
  const activeScopes = 'xui:import' in value ? [...scopes, await createImportedScope(value['xui:import'], Boolean(value['xui:standalone']), page)] : scopes
