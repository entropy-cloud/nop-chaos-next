...
describe('fetchAmisRequest', () => {
  beforeEach(() => {
    ...
    registerAmisRuntimeAdapter({
      getI18n: () => ({
        t: (_key: string, options?: { defaultValue?: string }) => options?.defaultValue || 'translated'
      }) as never,
      getLocale: () => 'en-US',
      getCurrentUser: () => null,
      getAuthToken: () => 'token',
  ...
  it('converts blob json payloads back into object payloads', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        new Response(new Blob([JSON.stringify({ status: 0, msg: 'ok', data: { id: 1 } })], { type: 'application/json' }), {
          status: 200,
          headers: {
