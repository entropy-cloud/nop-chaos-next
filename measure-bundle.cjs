...
console.log(
  `AMIS size: ${(amisFiles.reduce((sum, f) => sum + f.size, 0) / 1024 / 1024).toFixed(2)} MB`,
);

console.log(`\nFlux files: ${fluxFiles.length}`);
console.log(
  `Flux size: ${(fluxFiles.reduce((sum, f) => sum + f.size, 0) / 1024 / 1024).toFixed(2)} MB`,
);

console.log(`\nVendor files: ${vendorFiles.length}`);
console.log(
  `Vendor size: ${(vendorFiles.reduce((sum, f) => sum + f.size, 0) / 1024 / 1024).toFixed(2)} MB`,
);

console.log(`\nHost files: ${hostFiles.length}`);
console.log(
  `Host size: ${(hostFiles.reduce((sum, f) => sum + f.size, 0) / 1024 / 1024).toFixed(2)} MB`,
);

console.log(`\nPage files: ${pageFiles.length}`);
console.log(
  `Page size: ${(pageFiles.reduce((sum, f) => sum + f.size, 0) / 1024 / 1024).toFixed(2)} MB`,
);
...
largestFiles.forEach((file, index) => {
  console.log(
    `${index + 1}. ${file.name}: ${file.sizeMB.toFixed(2)} MB (${file.sizeKB.toFixed(2)} KB)`,
  );
});

console.log('\n--- Core Runtime Files (Always Loaded) ---');
const coreFiles = jsFiles.filter(
  (file) =>
...
console.log(
  `Core size: ${(coreFiles.reduce((sum, f) => sum + f.size, 0) / 1024 / 1024).toFixed(2)} MB (${(coreFiles.reduce((sum, f) => sum + f.size, 0) / 1024).toFixed(2)} KB)`,
);
