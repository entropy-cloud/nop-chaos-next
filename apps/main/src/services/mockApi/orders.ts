    ...
    logistics: [
      {
        ...
        eta: '2026-04-01',
        note: '优先派送至前台',
        timeline: ['03-29 18:00 已揽收', '03-30 02:20 到达上海转运中心', '03-30 08:15 派送中'],
      },
    ],
  },
  {
    ...
    logistics: [
      {
        id: 'log-3',
        company: 'YTO',
        trackingNo: 'YT99881122',
        shippingStatus: 'delivered',
        eta: '2026-03-22',
        note: '客户已签收',
        timeline: ['03-20 10:00 已发货', '03-21 09:10 运输中', '03-22 16:30 已签收'],
      },
    ],
  },
];
...
export async function saveOrderDetail(order: OrderRecord): Promise<OrderRecord> {
  const nextOrders = getOrderDocuments().map((item) =>
    item.id === order.id ? { ...order, updatedAt: new Date().toISOString() } : item,
  );
