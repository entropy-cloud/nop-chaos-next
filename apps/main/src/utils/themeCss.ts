...
export function applyThemeToDocument(themeConfig: ThemeConfig): void {
  ...
  const resolvedThemeId = hasRegisteredTheme(normalizedThemeId) ? normalizedThemeId : getDefaultThemeId()

  if (import.meta.env.DEV && resolvedThemeId !== normalizedThemeId) {
    console.warn(`[theme] Theme "${normalizedThemeId}" is not registered. Falling back to "${resolvedThemeId}".`)
  }
