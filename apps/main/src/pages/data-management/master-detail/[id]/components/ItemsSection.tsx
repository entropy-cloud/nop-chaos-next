...
export function ItemsSection({ filteredItems, dirty, errors, restoreItems, setDraft, markDirty, updateItem, removeItem }: ItemsSectionProps) {
    ...
    <Card className="theme-card">
      <CardHeader className="flex flex-row items-center justify-between gap-3">
        <CardTitle className="flex items-center gap-2">
          ...
          {dirty ? <span className="inline-flex h-2.5 w-2.5 rounded-full bg-[hsl(var(--danger))]" /> : null}
        </CardTitle>
          ...
          <Table>
            ...
            <TableBody>
              {filteredItems.map((item) => {
                ...
                return (
                  <TableRow key={item.id} className={cn(getTableRowClassName('subtle'), dirty && 'bg-[color-mix(in_hsl,hsl(var(--warning))_6%,transparent)]')}>
                    <TableCell>
