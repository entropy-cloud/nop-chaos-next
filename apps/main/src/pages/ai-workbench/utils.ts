...
export function buildHistoricalMessages(
  t: Translate,
  session: WorkbenchSession,
): WorkbenchMessage[] {
  const assistant =
    assistantCatalog.find((item) => item.id === session.assistantId) ?? assistantCatalog[0];
  ...
  return [
    ...
    },
    {
      ...
      role: 'assistant',
      content: t('aiWorkbench.history.assistantReply', {
        assistant: getAssistantName(t, assistant),
      }),
      createdAt: t('aiWorkbench.timestamps.earlier'),
    },
    {
