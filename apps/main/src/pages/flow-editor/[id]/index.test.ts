...
describe('flow editor helpers', () => {
  ...
  it('records node history only for meaningful changes', () => {
    expect(shouldRecordNodeChangeHistory([{ id: 'node-1', type: 'select', selected: true }])).toBe(false)
    expect(shouldRecordNodeChangeHistory([{ id: 'node-1', type: 'position', position: { x: 10, y: 10 }, dragging: true }])).toBe(false)
    expect(shouldRecordNodeChangeHistory([{ id: 'node-1', type: 'position', position: { x: 10, y: 10 }, dragging: false }])).toBe(true)
  })

  it('records edge history only when edge data changes', () => {
    expect(shouldRecordEdgeChangeHistory([{ id: 'edge-1', type: 'select', selected: true }])).toBe(false)
