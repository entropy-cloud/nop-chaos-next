...
export function useFlowHistory(initialSnapshot?: FlowStateSnapshot) {
  const [history, setHistory] = useState<FlowStateSnapshot[]>(initialSnapshot ? [initialSnapshot] : [])
  ...
  const recordSnapshot = useCallback((nodes: FlowNode[], edges: FlowEdge[]) => {
    setHistory((current) => {
      ...
      const next = [...base, { nodes: cloneNodes(nodes), edges: cloneEdges(edges) }].slice(-MAX_HISTORY_SIZE)
  ...
  return { history, historyIndex, historyIndexRef, canUndo, canRedo, initializeHistory, recordSnapshot, undo, redo }
}
