...
import type { FlowDocument } from '../../../../services/mock-api';
    ...
    <>
              ...
              <div className="space-y-4">
                <div className="rounded-lg border border-[hsl(var(--border))] bg-white/40 p-4 text-sm dark:bg-slate-900/25">
                  ...
                  <div className="meta-text mt-3 flex flex-wrap items-center gap-2">
                    <Badge
                      variant={
                        flowDocument?.status === 'enabled'
                          ? 'success'
                          : flowDocument?.status === 'disabled'
                            ? 'outline'
                            : 'warning'
                      }
                    >
                      {flowDocument?.status
                        ? t(`common.flowStatuses.${flowDocument.status}`)
                        : null}
                    </Badge>
                ...
                <div className="rounded-lg border border-[hsl(var(--border))] bg-white/40 p-4 dark:bg-slate-900/25">
                  {selectedNode ? (
                    ...
                    <EdgeInspector selectedEdge={selectedEdge} updateEdgeData={updateEdgeData} />
                  ) : (
      ...
      >
        <div className="flex h-full flex-col items-center justify-center gap-2 text-sm font-medium text-foreground">
          ...
          <span className="writing-mode-vertical text-sm tracking-[0.18em] [writing-mode:vertical-rl]">
            {inspectorHandleLabel}
          </span>
        </div>
      </Button>
