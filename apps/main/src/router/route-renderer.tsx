...
/* eslint-disable react-hooks/static-components -- route renderer dynamically resolves page components */
...
import { ensurePluginSharedModules } from '../plugins/shared-modules';
import { usePluginStore } from '../store/plugin-store';
import { ForbiddenPage, ServerErrorPage, getBuiltinPage } from './page-registry';

const AmisRouteRenderer = lazy(async () => {
  const { ensureAmisRuntime } = await import('../amis/init');
  ensureAmisRuntime();
  const module = await import('../amis/amis-route-renderer');
...
const FluxRouteRenderer = lazy(async () => {
  ...
  ensureFluxRuntime();
  const module = await import('../flux/flux-route-renderer');
